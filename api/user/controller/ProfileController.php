...
class ProfileController extends RestUserBaseController
{
	...
	public function editHeading(){
		$url = request()->param('url');
		
		if(empty($url))  $this->error('请上传图片');
		
		$userId = $this->getUserId();
		Db::name('user')->where('id',$userId)->update(['avatar'=>$url]);
    ...
    public function userInfo($field = '')
    {
        ...
        if ($this->request->isGet()) {
			...
			$model = new UserModel;
            if (empty($field)) {
                $userData = $model->field($fieldStr)->find($userId);
            } else {
                ...
                if (count($mixedField) > 1) {
                    ...
                    $userData = $model->where('id', $userId)->value($mixedField);
                }
            }
	...
	public function getCount(){
		...
		$userId   = $this->getUserId();
		$result   = Db::name('user')->field('balance,coin,is_vip')->where('id',$userId)->find();
        $result['balance'] = del0($result['balance']);
	...
	public function msg(){
		...
		if(empty($param['title'])) 		$this->error('请填写店铺名称！');
		if(empty($param['address'])) 	$this->error('请填写店铺地址！');
		if(empty($param['name'])) 		$this->error('请填写老板名称！');
		if(empty($param['phone'])) 		$this->error('请填写联系电话！');
		if(empty($param['type_name'])) 	$this->error('请选择合作类型！');
		...
		$result= Db::name('plugin_guestbook')->insert($data);
		if ($result) {
			$this->success("提交成功！");
		} else {
			$this->error("提交失败！");
