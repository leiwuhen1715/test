...
class ApiService {
    ...
    public static function terms($term_id=0){
        ...
        $terms=$terms_model->field('name,id,cat_img')->where($where)->order('list_order asc')->select()->ToArray();

        $mterms=array();
        
        foreach ($terms as $t){
            $cat=$terms_model->field('name,id,cat_img')->where(["is_show"=>1,'parent_id'=>$t['id']])->order('list_order asc')->select();
